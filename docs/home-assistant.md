...
>
> Note that **entering** `off` always does one poll, even with `BIRDFY_OFF_POLL_SECONDS=0`. This matters when the bridge *boots straight into* `off` (a restart while the persisted mode is off): the MQTT task — which publishes HA discovery and the device's `online` availability — is only started after the first device fetch, so without that entry poll a cold start in `off` mode would leave every entity showing **`unavailable`** in HA. The entry poll brings MQTT up with real state; `BIRDFY_OFF_POLL_SECONDS=0` then just disables the *repeat* polling.
