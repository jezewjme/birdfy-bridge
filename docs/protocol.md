...
**A/V drift caveat:** `setts` labels every frame exactly `1/FRAME_RATE` apart, while the audio rides its true 8 kHz µ-law sample clock. If `BIRDFY_FRAME_RATE` differs from the camera's real delivered rate (~8.6 fps measured, autoBitrate-driven — the SDP's 15 fps is the encoder ceiling), the video media clock runs fast/slow against audio and the two drift apart over a long session (a fixed `9` against a measured `8.6` ran ~4.5% fast — minutes of skew over an hour). The default is therefore the measured **`8.6`** (a float; `SETTS_TICK = round(90000/8.6) = 10465`), which brings the drift to the autoBitrate wobble floor. It can't reach exactly zero without re-encoding the audio (`aresample=async`) to actively track the video clock — a deliberate tradeoff to keep the audio path pure `-c:a copy`. Keep `BIRDFY_FRAME_RATE` on your camera's measured delivered rate.
